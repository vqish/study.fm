...
import { Search, MapPin, UserPlus, Users, MessageCircle, Clock, Loader2 } from 'lucide-react';
...
const UserCard = ({ user }: { user: UserProfile }) => {
  const isOnline = Date.now() - user.lastActive < 1000 * 60 * 10; // Active in last 10 min

  return (
    <div className="glass-panel" style={{ padding: '2rem', display: 'flex', flexDirection: 'column', gap: '1.5rem', position: 'relative', overflow: 'hidden', borderRadius: '24px', transition: 'transform 0.3s cubic-bezier(0.16, 1, 0.3, 1)' }} onMouseOver={e => e.currentTarget.style.transform='translateY(-5px)'} onMouseOut={e => e.currentTarget.style.transform='translateY(0)'}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
        {user.photoURL ? (
          <img src={user.photoURL} alt={user.displayName} style={{ width: '70px', height: '70px', borderRadius: '20px', objectFit: 'cover', border: '2px solid rgba(187,134,252,0.3)' }} />
        ) : (
        ...
        <div>
          <h3 style={{ fontSize: '1.25rem', fontWeight: 700, margin: 0 }}>{user.displayName}</h3>
          <p style={{ color: 'var(--accent-color)', fontSize: '0.9rem', marginTop: '0.2rem', fontWeight: 600 }}>{user.major}</p>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
         <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
            ...
            <span style={{ fontWeight: 500 }}>{user.country}</span>
         </div>
         <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginTop: '0.25rem' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.3rem' }}>
                <Clock size={14} color="var(--text-secondary)" />
                <span style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: '1rem' }}>{Math.floor(user.studyTime / 60)}h</span>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.3rem' }}>
                ...
                <span style={{ fontWeight: 700, color: 'var(--text-primary)', fontSize: '1rem' }}>{user.sessions}</span>
      ...
      <button 
        style={{ width: '100%', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', padding: '0.85rem', borderRadius: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.6rem', transition: 'all 0.2s', fontWeight: 600 }} 
        onMouseOver={e => { e.currentTarget.style.background = 'var(--accent-color)'; e.currentTarget.style.border = '1px solid var(--accent-color)'; }} 
        onMouseOut={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.border = '1px solid rgba(255,255,255,0.1)'; }}
      >
        <UserPlus size={18} /> Send Message
      </button>
    </div>
  );
};
